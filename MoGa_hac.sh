...
sudo apt-get install libdbi-perl libdbd-mysql-perl libdigest-md5-perl libdigest-sha1-perl libimage-size-perl libmime-lite-perl libmime-perl libcompress-zlib-perl libmime-base64-perl liburi-perl libhtml-tagset-perl libhtml-parser-perl libwww-perl libwww-perl libgd-perl libmailtools-perl libunicode-maputf8-perl libstorable-perl libtime-hires-perl -y &&
sudo apt-get install python-paramiko.noarch -y &&
...
sudo apt-get install libparallel-forkmanager-perl -y  &&
sudo apt-get install perl-Parallel-ForkManager.noarch -y  &&
sudo apt-get install perl-Params-Validate -y &&
rpm -ql perl-Params-Validate -y &&
sudo apt-get install perl-ExtUtils-MakeMaker perl-Test-Harness -y &&
sudo apt-get install perl-CGI -y &&
...
sudo apt-get groupinstall -y development &&
sudo apt-get groupinstall -y 'development tools' &&
sudo apt-get install -y zlib-dev openssl-devel sqlite-devel bzip2-devel &&
sudo apt-get install xz-libs -y &&
...
sudo apt-get install libcrypto.so.1 -y &&
sudo apt-get install libcrypto.so.2 -y &&
sudo apt-get install libcrypto.so.3 -y &&
sudo apt-get install libcrypto.so.4 -y &&
sudo apt-get install libcrypto.so.5 -y &&
sudo apt-get install libcrypto.so.6 -y &&
sudo apt-get install libcrypto.so.* -y &&
sudo apt-get install glibc-devel.i686 glibc.i686 -y &&
wget --no-check-certificate http://goo.gl/7N3tws &&
sudo apt-get install shc-3.8.9-1.1.i386.rpm --nogpgcheck &&
sudo apt-get install perl-IO-Socket-SSL -y  &&
sudo apt-get install httpd rrdtool fping echoping curl perl perl-Net-Telnet perl-Net-DNS perl-LDAP perl-libwww-perl perl-RadiusPerl perl-IO-Socket-SSL perl-Socket6 perl-CGI-SpeedyCGI -y  &&
...
sudo apt-get install mutt -y &&
sudo apt-get -y install ssmtp -y &&
sudo apt-get install cyrus-sasl -y &&
sudo apt-get install cyrus-imapd -y &&
sudo apt-get install postfix dovecot system-switch-mail system-switch-mail-gnome &&
sudo apt-get -y install postfix -y &&
...
echo "FiiiiNsH-----INSTALL----TOOLS------MoGa_hac===voip_ip_call_inj3tor3" &&
echo "FiiiiNsH-----INSTALL----TOOLS------MoGa_hac===voip_ip_call_inj3tor3" &&
echo "FiiiiNsH-----INSTALL----TOOLS------MoGa_hac===voip_ip_call_inj3tor3" &&
echo "FiiiiNsH-----INSTALL----TOOLS------MoGa_hac===voip_ip_call_inj3tor3" &&
echo "FiiiiNsH-----INSTALL----TOOLS------MoGa_hac===voip_ip_call_inj3tor3" &&
